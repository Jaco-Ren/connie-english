...
| 审核流程 | Jaco 可通过、退回、撤销通过、为过期任务开放补交，并停止 Connie 自定义任务的未来安排；只有 `approved` 任务计入积分 |
...
3. 打开证明图片检查完成情况。
4. 选择通过或不通过。
5. 必要时撤销通过、停止自定义任务的未来安排、手动加分/扣分。
6. 给 Connie 写当天提醒或反馈。
...
- Jaco 可以审核、撤销通过、为过期任务开放补交、停止 Connie 自定义任务的未来安排且不删除历史积分、创建或删除手动调分记录。
